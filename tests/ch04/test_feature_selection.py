...
from ch04.feature_selection import FFS, EFS
...
class FFSTest(unittest.TestCase):
    ...
    def setUp(self):
        self.X, self.y = create_test_dataset(0)
...
class EFSTest(unittest.TestCase):
    ...
    def test_select_the_most_predictive_feature(self):
        knn = KNeighborsClassifier(n_neighbors=5)
        efs = EFS(knn, k_features=1).fit(self.X, self.y)

        self.assertEqual(len(efs.subsets_), 1)
        self.assertEqual(efs.subsets_[0][0], 0)

    def test_select_model_with_two_the_most_predictive_features(self):
        knn = KNeighborsClassifier(n_neighbors=5)
        efs = EFS(knn, k_features=2).fit(self.X, self.y)

        self.assertEqual(len(efs.subsets_), 2)

        self.assertCountEqual(efs.subsets_[0], [0])
        self.assertCountEqual(efs.subsets_[1], [0, 1])

    def test_for_selecting_more_features_than_in_dataset(self):
        knn = KNeighborsClassifier(n_neighbors=5)
        efs = EFS(knn, k_features=10).fit(self.X, self.y)

        self.assertEqual(len(efs.subsets_), 3)